...
int main() {
    ...
    if (r < 0) {
        cout
            << "Hubungan antara variabel X dan Y adalah NEGATIF, dimana jika nilai "
            "X bertambah, maka nilai Y pun akan MENGECIL, dan juga sebaliknya";
    }

    if (r > 0) {
        cout
            << "Hubungan antara variabel X dan Y adalah POSITIF, dimana jika nilai "
            "X bertambah, maka nilai Y pun akan BERTAMBAH, dan juga sebaliknya";
    }
...
void ZigmaXY(int hasil, int kali, int i, int n, int yindeks[i],
    int xindeks[i]) {
...
void HasilR(int i, int n, int r, int kali, int jumlahx, int jumlahy,
    int pangkatx, int pangkaty) {

    int akhir = 0;

    for (i = 0; i <= n; i++) {
        r = kali - jumlahx * jumlahy / sqrt(pangkatx - pangkatx) *
            sqrt(pangkaty - pangkaty);
    }
    return akhir = 0;
}